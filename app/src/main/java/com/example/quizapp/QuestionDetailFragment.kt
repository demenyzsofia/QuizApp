...
class DetailFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        view?.apply {
            val item: Item = viewModel.getItem()

            initializeView(this)

            questionText.setText(item.text1)
            ans1.setText(item.text2)
            val index=viewModel.currentPosition
            ans2.setText(viewModel.questions[index].answers[1])
            ans3.setText(viewModel.questions[index].answers[2])
            ans4.setText(viewModel.questions[index].answers[3])
